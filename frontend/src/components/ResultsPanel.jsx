...
import { AlertTriangle, ChevronDown, HelpCircle, Check, Clipboard, Share, Loader, Download } from 'lucide-react';
...
const ResultsPanel = ({ detections, image }) => {
        ...
        return (
            <div className="bg-white rounded-xl shadow-lg p-6 h-full">
                <div className="flex items-center justify-center flex-col h-full text-center p-8">
                    ...
                    <h3 className="text-xl font-medium text-gray-700 mb-2">No Issues Detected</h3>
                    <p className="text-gray-500">
                        Our system couldn't identify any specific diseases or issues with this plant image.
                    </p>
                </div>
            </div>
        );
    ...
    // Get the most confident detection for primary display
    const primaryDetection = sortedDetections[0];
    const conditionName = primaryDetection.class_name;
    ...
    useEffect(() => {
        const fetchAIAnalysis = async () => {
          ...
          You are an expert in plant pathology and gardening. In this plant image, we've detected "${conditionName}" with ${(primaryDetection.confidence * 100).toFixed(1)}% confidence.
    ...
    }, [conditionName, primaryDetection.confidence]);
        ...
        return (
            <div className="bg-white rounded-xl shadow-lg p-6 h-full">
                <div className="flex items-center justify-center flex-col h-full text-center p-8">
                    ...
                    <h3 className="text-xl font-medium text-gray-700 mb-2">Analyzing Plant Condition</h3>
                    <p className="text-gray-500">
        ...
        return (
            <div className="bg-white rounded-xl shadow-lg p-6 h-full">
                <div className="flex items-center justify-center flex-col h-full text-center p-8">
                    ...
                    <h3 className="text-xl font-medium text-gray-700 mb-2">Analysis Error</h3>
                    <p className="text-gray-500">
                        {error}. The condition detected was "{conditionName}" with
                        {(primaryDetection.confidence * 100).toFixed(1)}% confidence.
                    </p>
                </div>
            </div>
        );
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden h-full">
            ...
            <div className="bg-emerald-700 p-6 text-white">
                <div className="flex justify-between items-center">
                    ...
                    <div className="text-emerald-200 text-sm">
                        Main detection: {conditionName} ({(primaryDetection.confidence * 100).toFixed(1)}%)
                    </div>
                </div>
            </div>
            ...
            {sortedDetections.length > 1 && (
                <div className="border-t border-gray-200">
                    <div className="p-4">
                        <h3 className="text-lg font-medium text-emerald-800 mb-2">Other Possible Conditions</h3>
                        <ul className="space-y-2">
                            {sortedDetections.slice(1).map((detection, index) => (
                                <li key={index} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                                    <span className="font-medium text-gray-700">{detection.class_name}</span>
            ...
            <div className="border-t border-gray-200 p-4 flex flex-wrap gap-3">
                ...
                <button
                    className="flex items-center px-4 py-2 bg-emerald-100 hover:bg-emerald-200 rounded-lg text-emerald-700 text-sm transition-colors"
                >
                ...
                <button
                    className="flex items-center px-4 py-2 bg-blue-100 hover:bg-blue-200 rounded-lg text-blue-700 text-sm transition-colors ml-auto"
                >
